 ...
 */
export const splitFirstSpace = (str: string): [string, string] => {
 ...
 */
export const splitByTags = (str: string): string[] => {
	if (!str) return [];
 ...
 */
export const createPipeSplitter = (leadingCharacter: string): ((str: string) => string[]) => {
 ...
 */
export const defaultTagInfo: TagInfo = {
	getStripped: (_tag: string, text: string): string => splitTagByPipe(text)[0] || "",
};
 ...
 */
export const createSimpleTagInfo = (): TagInfo => ({
	getStripped: (_tag: string, text: string): string => splitTagByPipe(text)[0] || "",
});
 ...
 */
export const createDiceTagInfo = (): TagInfo => ({
	getStripped: (_tag: string, text: string): string => {
 ...
 */
export const builtInTags: TagLookup = {
 ...
 */
export const hasTags = (str: string): boolean => {
	if (!str) return false;
	return /{[@=][^}]+}/.test(str);
};
 ...
 */
export const getTagContent = (tag: string): string | null => {
	if (!isTag(tag)) return null;
 ...
 */
export const parseTag = (tagStr: string): ParsedTag | null => {
	if (!isTag(tagStr)) return null;
...
export const buildTag = (
	...
	if (displayText) {
		if (!source) parts.push(""); // Ensure source slot exists
