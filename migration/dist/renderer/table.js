 ...
 */
export var RollColMode;
(function (RollColMode) {
    RollColMode[RollColMode["NONE"] = 0] = "NONE";
    RollColMode[RollColMode["STANDARD"] = 1] = "STANDARD";
    RollColMode[RollColMode["EXACT"] = 2] = "EXACT";
    RollColMode[RollColMode["RESULT"] = 3] = "RESULT";
})(RollColMode || (RollColMode = {}));
// ============ Constants ============
 ...
 */
export const getHeaderRowMetas = (ent) => {
    if (!ent.colLabels?.length && !ent.colLabelRows?.length)
        return null;
    // Simple case: single row of column labels
    if (ent.colLabels?.length)
        return [ent.colLabels];
    // Multiple header rows
    const colLabelRows = ent.colLabelRows;
    ...
    if (lenPer.every(len => len === lenMax))
        return colLabelRows;
 ...
 */
export const getHeaderRowSpanWidth = (colLabelRow) => {
    return colLabelRow.reduce((acc, cell) => {
 ...
 */
export const isRollableCell = (cell) => {
    if (cell == null)
        return false;
 ...
 */
export const isEveryRowRollable = (rows) => {
 ...
 */
export const getAutoConvertedRollMode = (table, options = {}) => {
    ...
    if (!headerRowMetaBottom || headerRowMetaBottom.length < 2)
        return RollColMode.NONE;
    ...
    if (!table.rows || !isEveryRowRollable(table.rows)) {
        return RollColMode.NONE;
    }
    return rollColMode;
};
 ...
 */
export const getColRollType = (cell) => {
    if (cell == null)
        return null;
    let text;
    if (typeof cell === "string") {
        ...
        const headerCell = cell;
        if (headerCell.isRoller)
            return RollColMode.STANDARD;
 ...
 */
export const parseRollCell = (value) => {
    if (value == null)
        return null;
    // Already parsed
 ...
 */
export const convertRowsToRollable = (rows) => {
    return rows.map(row => {
        const cells = getRowCells(row);
        if (!cells || !cells.length)
            return cells ?? [];
 ...
 */
export const getColumnCount = (table) => {
    // Try column labels first
    if (table.colLabels?.length)
