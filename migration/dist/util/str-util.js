...
let _TITLE_LOWER_WORDS_RE = null;
let _TITLE_UPPER_WORDS_RE = null;
let _TITLE_UPPER_WORDS_PLURAL_RE = null;
const initTitleCase = () => {
...
// ============ Padding ============
export const padNumber = (n, len, padder = "0") => {
    return String(n).padStart(len, padder);
};
// ============ Truncation ============
export const ellipsisTruncate = (str, atLeastPre = 5, atLeastSuff = 0, maxLen = 20) => {
...
// ============ URL Encoding ============
export const encodeApos = (str) => str.replace(/'/g, "%27");
export const toUrlified = (str) => encodeURIComponent(str.toLowerCase()).toLowerCase();
// ============ String Utilities ============
export const escapeRegexp = (str) => str.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&");
export const isNumeric = (str) => !isNaN(parseFloat(str)) && isFinite(Number(str));
...
// ============ Chunking ============
export const toChunks = (str, size) => {
...
export const trimAnyChar = (str, chars) => {
    let start = 0;
    let end = str.length;
...
// ============ Counting ============
export const countSubstring = (str, term) => {
...
const IRREGULAR_SINGLE_WORDS = Object.fromEntries(Object.entries(IRREGULAR_PLURAL_WORDS).map(([k, v]) => [v, k]));
const IRREGULAR_SINGLE_PATTERNS = [
    [/(axe)s$/i, "$1"],
];
const getMatchedCase = (original, replacement) => {
    if (original === original.toLowerCase())
        return replacement.toLowerCase();
    if (original === original.toUpperCase())
        return replacement.toUpperCase();
    if (original[0] === original[0].toUpperCase())
        return uppercaseFirst(replacement);
    return replacement;
};
export const toSingle = (str) => {
    ...
    if (/[bcdfghjklmnpqrstvwxyz]ies$/i.test(str))
...
// ============ Duplicate Naming ============
export const getNextDuplicateName = (str) => {
    if (str == null)
        return null;
