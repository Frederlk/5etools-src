...
// ============ Utility Functions ============
const getNextPrefix = (options, prefix = "") => {
...
export const defaultMarkdownRendererConfig = {
    styleHint: "classic",
    isAddHandlers: false,
    baseUrl: "",
...
export class MarkdownRenderer extends BaseRenderer {
    ...
    _renderEntriesSubtypes(entry, textStack, meta, options, incDepth = true) {
        ...
        if (entry.entries) {
            ...
            for (let i = 0; i < len; ++i) {
                ...
                this._recursiveRender(entry.entries[i], textStack, meta, { prefix: isFirstInline ? "" : getNextPrefix(options), suffix });
            }
    ...
    _renderTable(entry, textStack, meta, options) {
        ...
        let labelRows = headerRowMetas ? [...headerRowMetas] : [];
        ...
        const mdHeaderRows = labelRows.map(labelRow => {
            return labelRow.flatMap((entCellHeader) => {
                ...
                if (!cntPadCells)
                    return [ptCellPrimary];
                return [ptCellPrimary, " ".repeat(cntPadCells)];
            });
        });
        const widths = [...new Array(Math.max(...mdHeaderRows.map(mdHeaderRow => mdHeaderRow.length)))]
            .map((_, i) => {
            return Math.max(...mdHeaderRows.map(mdHeaderRow => (mdHeaderRow[i] || "").length), this._getPaddedStyleText({ style: styles[i] || "" }).length);
        });
        const mdTable = [];
        ...
        const mdHeaderRowsPadded = mdHeaderRows.map(mdHeaderRow => {
            return mdHeaderRow.map((header, ixCell) => this._getPaddedTableText({ text: header, width: widths[ixCell], ixCell, styles }));
        });
        const mdStyles = [];
        if (styles.length) {
    ...
    _renderQuote(entry, textStack, meta, options) {
        ...
        const len = entry.entries.length;
        for (let i = 0; i < len; ++i) {
            this._recursiveRender(entry.entries[i], textStack, meta, { prefix: getNextPrefix(options, "*"), suffix: "*" });
            if (i !== len - 1)
                textStack[0] += "\n\n";
        }
    ...
    _getDiceDisplayText(entry, name) {
    ...
    _renderActions(entry, textStack, meta, options) {
        ...
        meta.depth = 2;
        this._renderEntriesSubtypes({ ...entry, type: "entries" }, textStack, meta, options);
...
// ============ Factory Functions ============
export const createMarkdownRenderer = (config = {}) => {
    return new MarkdownRenderer(config);
};
let _sharedMarkdownRenderer = null;
...
export const markdownUtils = {
    ...
    getNormalizedNewlines(str) {
        return str.replace(/\n\n+/g, "\n\n");
    },
    getRenderedAbilityScores(ent, opts = {}) {
        const { prefix = "" } = opts;
