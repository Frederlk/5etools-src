...
import { MarkdownConverter, getEntries, getConvertedTable, postProcessTable, ENTRIES_WITH_CHILDREN, ENTRIES_WITH_ENUMERATED_TITLES, } from "./converter.js";
describe("converter", () => {
...
> *Medium beast*`;
                ...
                expect(creature.name).toContain("Text Converter");
            });
...
This has **bold text** in it.`;
                const result = MarkdownConverter.getEntries(md);
                const entries = result[0];
                const content = entries.entries?.find(e => typeof e === "string" && e.includes("{@b"));
...
This has *italic text* here.`;
                const result = MarkdownConverter.getEntries(md);
                const entries = result[0];
                const content = entries.entries?.find(e => typeof e === "string" && e.includes("{@i"));
                expect(content).toBeDefined();
            });
...
This has ~~struck text~~ here.`;
                const result = MarkdownConverter.getEntries(md);
                const entries = result[0];
                const content = entries.entries?.find(e => typeof e === "string" && e.includes("{@s"));
                expect(content).toBeDefined();
            });
...
Check [this link](http://example.com) out.`;
                const result = MarkdownConverter.getEntries(md);
                const entries = result[0];
                const content = entries.entries?.find(e => typeof e === "string" && e.includes("{@link"));
                expect(content).toBeDefined();
            });
        });
...
Column 2`;
    ...
    describe("postProcessTable", () => {
        it("should normalize cell counts", () => {
            ...
            expect(tbl.rows[0].length).toBe(3);
            expect(tbl.rows[1].length).toBe(3);
        });
