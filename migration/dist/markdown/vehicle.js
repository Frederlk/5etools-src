...
// ============ Ship Helpers ============
const getVehicleShipRenderableEntriesMeta = (ent) => {
    ...
    const crewPassengerParts = [];
    if (ent.capCrew != null)
        crewPassengerParts.push(`**Crew** ${ent.capCrew}`);
    if (ent.capPassenger != null)
        crewPassengerParts.push(`**Passengers** ${ent.capPassenger}`);
    ...
    const entryTravelPace = ent.pace != null
        ...
        : undefined;
    const entriesOtherActions = [];
    const entriesOtherOthers = [];
    if (ent.other?.length) {
...
const getSectionHpEntriesMeta = (opts) => {
    ...
    if (entry.hp != null) {
        ...
        if (entry.hpNote)
            hpStr += ` ${entry.hpNote}`;
        if (entry.dt != null)
            hpStr += `; damage threshold ${entry.dt}`;
    }
...
const getSpelljammerRenderableEntriesMeta = (ent) => {
    const rows = [];
    if (ent.capCrew != null) {
        let crewStr = String(ent.capCrew);
        if (ent.capCrewNote)
            crewStr += ` ${ent.capCrewNote}`;
...
const getElementalAirshipRenderableEntriesMeta = (ent) => {
    const rows = [];
    if (ent.capCrew != null) {
        let crewStr = String(ent.capCrew);
        if (ent.capCrewNote)
...
const formatSpeed = (speed) => {
    ...
    if (speed.burrow != null)
        parts.push(`burrow ${speed.burrow} ft.`);
    if (speed.climb != null)
        parts.push(`climb ${speed.climb} ft.`);
    return parts.join(", ") || "\u2014";
};
const getSpelljammerStationEntriesMeta = (entry) => {
    return {
...
const getElementalAirshipStationEntriesMeta = (entry) => {
    return {
        entryName: entry.name,
        entryArmorClass: entry.ac != null ? `**Armor Class** ${entry.ac}` : null,
        entryHitPoints: entry.hp != null ? `**Hit Points** ${entry.hp}` : null,
...
const getVehicleInfwarRenderableEntriesMeta = (ent) => {
    const sizeStr = getSizeString(ent.size);
...
const shipHelpers = {
    getCrewCargoPaceSection_(ent, opts = {}) {
    ...
    getMovementSection_(opts) {
        ...
        if (entry.locomotion?.length) {
            for (const loco of entry.locomotion) {
                ...
                parts.push(renderer.render(locoEntry));
            }
        }
        if (entry.speed?.length) {
            for (const spd of entry.speed) {
                ...
                parts.push(renderer.render(spdEntry));
            }
        }
        return parts
            .map(it => it != null ? it.trim() : it)
        ...
        if (entry.action?.length) {
            for (const act of entry.action) {
                if (act.entries?.length) {
                    parts.push(renderer.render({ type: "entries", name: act.name, entries: act.entries }, { meta: createRenderMeta({ depth: 2 }) }));
                }
            }
        }
        return parts
...
// ============ Trait Rendering ============
const getLinesRenderedTraits = (opts) => {
    const { ent, renderer } = opts;
    if (!ent.trait?.length)
        return [];
    const orderedTraits = [...ent.trait].sort((a, b) => {
    ...
    return [
        ...
        ...orderedTraits.map((entry) => renderer.render(entry, { meta: createRenderMeta({ depth: 2 }) })),
    ];
};
// ============ Vehicle Type Renderers ============
const getRenderedStringShip = (ent, opts) => {
    const renderer = getMarkdownRenderer();
...
const getRenderedStringSpelljammer = (ent, opts) => {
    const renderer = getMarkdownRenderer();
...
const getRenderedStringElementalAirship = (ent, opts) => {
    const renderer = getMarkdownRenderer();
...
const getRenderedStringInfwar = (ent, opts) => {
    const renderer = getMarkdownRenderer();
...
const getVehicleUpgradeCompactRenderedString = (ent, opts = {}) => {
    const renderer = getMarkdownRenderer();
    const entries = [
    ...
    const ptBody = entries
        ...
        if (typeof e === "string")
            return e;
        return renderer.render(e, { meta: createRenderMeta({ depth: 1 }) });
    })
        .filter(Boolean)
        .join("\n\n");
    return `${ptHeader}\n\n${ptBody}`.trim();
...
export class VehicleMarkdownRenderer {
    ...
    getCompactRenderedString(ent, opts = {}) {
        ...
        switch (vehicleType) {
            ...
            case "SPELLJAMMER":
                return getRenderedStringSpelljammer(vehicle, opts);
            case "ELEMENTAL_AIRSHIP":
                return getRenderedStringElementalAirship(vehicle, opts);
            case "INFWAR":
                return getRenderedStringInfwar(vehicle, opts);
            case "CREATURE":
...
// ============ Module Export ============
let _vehicleRenderer = null;
export const getVehicleMarkdownRenderer = (styleHint = "classic") => {
    if (!_vehicleRenderer) {
