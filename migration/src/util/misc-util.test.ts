...
import {
	...
	pDelay,
	getWalker,
	GENERIC_WALKER_ENTRIES_KEY_BLOCKLIST,
} from "./misc-util.js";
...
describe("misc-util", () => {
	...
	describe("set", () => {
		...
		it("should return null for null object", () => {
			expect(set(null as unknown as Record<string, unknown>, "a", 1)).toBe(null);
		});
	...
	describe("deleteObjectPath", () => {
		...
		it("should not delete non-empty parents", () => {
			const obj = { a: { b: { c: 1 }, d: 2 } } as Record<string, unknown>;
			deleteObjectPath(obj, "a", "b", "c");
	...
	describe("flatten", () => {
		...
		it("should handle primitives", () => {
			expect(flatten(5 as unknown)).toBe(5);
		});
	...
	describe("getWalker", () => {
		...
		describe("object handler", () => {
			...
			it("should throw on null return without allowDelete", () => {
				...
				expect(() =>
					walker.walk({ a: 1 }, {
						object: () => null as unknown as Record<string, unknown>,
					})
			...
			it("should allow null with isAllowDeleteObjects", () => {
				...
				const result = walker.walk({ a: { b: 1 } }, {
					object: (obj) => (Object.keys(obj).length ? obj : null as unknown as Record<string, unknown>),
				});
		...
		describe("array handler", () => {
			...
			it("should throw on null return without allowDelete", () => {
				...
				expect(() =>
					walker.walk({ a: [1] }, {
						array: () => null as unknown as unknown[],
					})
