...
describe("array-util", () => {
	...
	describe("joinConjunct", () => {
		...
		it("should use Oxford comma by default for 3+ elements", () => {
			expect(joinConjunct(["one", "two", "three"], ", ", " and ")).toBe(
				"one, two, and three"
			);
		});

		it("should omit Oxford comma when nonOxford=true", () => {
			expect(joinConjunct(["one", "two", "three"], ", ", " and ", true)).toBe(
				"one, two and three"
			);
		});
	...
	describe("equals", () => {
		...
		it("should handle both null/undefined", () => {
			expect(equals(null as unknown as [], null as unknown as [])).toBe(true);
		});

		it("should handle one null", () => {
			expect(equals([1], null as unknown as [])).toBe(false);
			expect(equals(null as unknown as [], [1])).toBe(false);
		});
	...
	describe("pSerialAwaitFirst", () => {
		it("should return first truthy mapped result", async () => {
			const result = await pSerialAwaitFirst([1, 2, 3], async (x) =>
				x > 1 ? x * 10 : undefined
			);
	...
	describe("pSerialAwaitReduce", () => {
		it("should reduce serially", async () => {
			const result = await pSerialAwaitReduce(
				[1, 2, 3, 4],
				async (acc, x) => acc + x,
				0
			);
